...
# Цель: Проверить корректность сортировки товаров от самой низкой цены к самой высокой
...
def test_sort_by_price():
    ...
    # --- Шаг 3: Выбрать сортировку по цене (от низкой к высокой) ---
    sort_dropdown = Select(driver.find_element(By.CLASS_NAME, "product_sort_container"))
    sort_dropdown.select_by_value("lohi")
    time.sleep(3)

    # --- Шаг 4: Получить все цены и проверить порядок ---
    prices = driver.find_elements(By.CLASS_NAME, "inventory_item_price")
    price_list = [float(p.text.replace("$", "")) for p in prices]
    print("Цены на странице:", price_list)

    # --- Проверка, что цены отсортированы по возрастанию ---
    assert price_list == sorted(price_list), " Ошибка сортировки"
    print("Товары отсортированы от низкой цены к высокой")
